...
int mdadm_mount(void)
{
	...
	if (ismounted == 1) // if ismounted return -1
		return -1;
	if (jbod_operation(op(0, 0, JBOD_MOUNT, 0), NULL) == 0) // if mount succeed ismounted=1 return 1
	{
		ismounted = 1;
		return 1;
	}
...
int mdadm_unmount(void)
{
	if (ismounted == 0) // if not ismounted return -1
		return -1;
	if (jbod_operation(op(0, 0, JBOD_UNMOUNT, 0), NULL) == 0) // if unmount succeed ismounted=0 return 1
	{
...
int mdadm_write_permission(void)
{
	...
	if (is_written == 1) // if ismounted return -1
		return -1;
	if (jbod_operation(op(0, 0, JBOD_WRITE_PERMISSION, 0), NULL) == 0) // if mount succeed ismounted=1 return 1
	{
		is_written = 1;
		return 1;
	}
...
int mdadm_revoke_write_permission(void)
{
	if (is_written == 0) // if not ismounted return -1
		return -1;
	if (jbod_operation(op(0, 0, JBOD_REVOKE_WRITE_PERMISSION, 0), NULL) == 0) // if unmount succeed ismounted=0 return 1
	{
...
int mdadm_read(uint32_t start_addr, uint32_t read_len, uint8_t *read_buf)
{
	...
	while (usedlen < read_len)
	{
		...
		jbod_operation(op(startDisk, 0, JBOD_SEEK_TO_DISK, 0), NULL); // reseek
		jbod_operation(op(0, startBlock, JBOD_SEEK_TO_BLOCK, 0), NULL);

		byteRemain = 256 - startByte; // how many byte left in block
		if (!cache_enabled() || cache_lookup(startDisk, startBlock, tempbuf) == -1)
		{
			jbod_operation(op(0, 0, JBOD_READ_BLOCK, 0), tempbuf);
			cache_insert(startDisk, startBlock, tempbuf);
		}
		// jbod_operation(op(0, 0, JBOD_READ_BLOCK, 0), tempbuf); // read the block into temper buffer
		...
		else // cant read in one block
		{
			...
			if (read_len - usedlen > 256) // more than one block
			{
				...
				jbod_operation(op(startDisk, 0, JBOD_SEEK_TO_DISK, 0), NULL); // reseek
				jbod_operation(op(0, startBlock, JBOD_SEEK_TO_BLOCK, 0), NULL);
				jbod_operation(op(0, 0, JBOD_READ_BLOCK, 0), tempbuf); // read the block
...
int mdadm_write(uint32_t start_addr, uint32_t write_len, const uint8_t *write_buf)
{
	...
	while (usedlen < write_len)
	{
		...
		if (byteRemain > write_len - usedlen) // can write in one block
		{
			...
			jbod_operation(op(0, 0, JBOD_WRITE_BLOCK, 0), tempbuf); // write the temper buffer into block
																	// cache_insert(startDisk, startBlock, tempbuf);
		}
